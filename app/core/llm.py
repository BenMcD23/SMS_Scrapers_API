...
Models are tried in order until one answers. Nemotron 3 Ultra on NVIDIA's free
endpoint leads: it benchmarks level with Gemini 3.5 Flash and its free tier
caps requests per minute (~40, shared across the whole API key) rather than per
day, so a month of texts never runs it dry — bursts just wait. Gemini 3.5 Flash writes
...
# Nemotron thinks hard; NVIDIA's own example uses 16k, and anything less risks
