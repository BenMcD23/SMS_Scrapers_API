...
Models are tried in order until one answers. GLM-5.3 on NVIDIA's free endpoint
leads: it is joint-top of the open-weights models on Artificial Analysis's index
(well ahead of Nemotron 3 Ultra, which led before), answers in ~10s, and NVIDIA's
free tier caps requests per minute (~40, shared across the whole API key) rather
than per day, so a month of texts never runs it dry — bursts just wait. Only one
NVIDIA model is in the chain: they all share that one key's limit, so a second
would fail at the same moment as the first. Gemini 3.8 Flash comes next but allows
only 20 requests/day, then Gemini 2.5 Flash (250/day), with Groq's gpt-oss-120b
as the final fallback. All are thinking models — keep token budgets high enough
that reasoning doesn't starve the actual answer.

Shared by every AI feature (parade-night texts, NCO appraisals, JI/AO
descriptions, inspection analysis) so the chain, the quota fallback and the
model labels are defined once. Callers get back the model id that actually
answered, so the UI can say a fallback was used.
"""
...
# GLM reasons before it answers; 16k (NVIDIA's own example budget) keeps the
# reasoning from eating the whole budget before the answer starts.
...
# back — NVIDIA's per-minute limit is waited out rather than fallen back on, so in
# practice that means NVIDIA errored or the key is missing.
MODEL_PREFERENCE = [NVIDIA_MODEL, *GEMINI_MODELS, GROQ_MODEL]
PRIMARY_MODEL = MODEL_PREFERENCE[0]

MODEL_LABELS = {
    NVIDIA_MODEL: "GLM 5.3",
    "gemini-3.8-flash": "Gemini 3.8 Flash",
    "gemini-2.5-flash": "Gemini 2.5 Flash",
    # No longer in the chain, but stored on rows generated before the switch.
...
def generate(prompt: str, system_prompt: str, *, temperature: float = 0.6,
             max_tokens: int = 8000, groq_max_tokens: int | None = None,
             models: list[str] | None = None) -> tuple[str, str]:
    """Try each model in preference order; return (output_text, model_id used).

    ``models`` overrides the chain for a caller whose prompt the default leader
    can't handle (see the NCO appraisals).
    ...
    for model in models or MODEL_PREFERENCE:
        try:
