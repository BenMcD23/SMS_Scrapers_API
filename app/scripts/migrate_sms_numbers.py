...
is so it keeps getting the texts.
    ...
    # from /app in the container (or the repo root with app/ on PYTHONPATH)
    python -m scripts.migrate_sms_numbers            # report only, changes nothing
    python -m scripts.migrate_sms_numbers --apply    # do it
    python -m scripts.migrate_sms_numbers --apply --keep-rows
